...
public class CircleProgress extends View {
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        neiYuanColor = typedArray.getColor(R.styleable.CircleProgress_neiYuanColor, ContextCompat.getColor(getContext(), R.color.transparent));
        ...
        ringColor = typedArray.getColor(R.styleable.CircleProgress_ringColor, ContextCompat.getColor(getContext(), R.color.white));
        ringProgressColor = typedArray.getColor(R.styleable.CircleProgress_ringProgressColor, ContextCompat.getColor(getContext(), R.color.blue));
        ...
        textColor = typedArray.getColor(R.styleable.CircleProgress_textColor, ContextCompat.getColor(getContext(), R.color.black));
        textSize = (int) typedArray.getDimension(R.styleable.CircleProgress_textSize, dip2px(getContext(), 17));
        typedArray.recycle();
        initPaint();
    }
...
//        drawRing(canvas);
    ...
    private void drawProgressRing(Canvas canvas) {
        ...
        LinearGradient linearGradient = new LinearGradient(0, 0, getMeasuredWidth(), getMeasuredHeight(),
                ringProgressColor, ringProgressSecondColor, Shader.TileMode.MIRROR);
        mPaint.setShader(linearGradient);
        if (isRound) mPaint.setStrokeCap(Paint.Cap.ROUND);
        float angle = (float) chuFa(viewProgress * getEffectiveDegree(), viewMax, 2);
        if (!isClockwise) angle = -1 * angle;
